...
THREE.ShaderLib[ 'sky' ] = {
	...
	uniforms: {
		...
		bottomColor: { value: new THREE.Color( 0x337700 ) },
		offset: { value: 0 },
		exponent: { value: 1.5 }
	...
	fragmentShader: [
		...
		"const float HORIZON_BLEND_SIZE = 0.02;",
		...
		"{",
			...
			//modfined from three.js example to have user definable ground color
			"float h = max( min( normalize( vWorldPosition + offset ).y / HORIZON_BLEND_SIZE, 1.0 ), 0.0);",
			
			"if(h <= 0.0) {",
				"gl_FragColor.rgb = bottomColor;",
				"gl_FragColor.a = 1.0;",
			"}",
			...
			"else {",
				...
				//modfined from three.js example to have user definable ground color
				"gl_FragColor.rgb = mix( bottomColor, retColor, max( pow( abs(h), exponent ), 0.0 ) );",
